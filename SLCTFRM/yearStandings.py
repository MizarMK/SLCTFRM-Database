...
def createStandings():
	...
	for row in leagueID:
		sql = "select distinct name from leagues where leagueID = %s"
		cur.execute(sql, (row)) 
		league = cur.fetchone()
		print()
		print(league[0])
		...
		if int(paramYear) < 1969:
			sql = "SELECT t.teamName, t.W, t.L, DivWin, WCWin, lgWin, WSWin FROM teams t, leagues l WHERE t.lgID = l.leagueID AND t.yearID = %s AND t.lgID = %s GROUP BY t.teamID"
